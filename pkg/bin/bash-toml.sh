...
if [ -d "$REPLY/lib/glue" ]; then
	BASH_TOML_LIB_DIR="$REPLY/lib/bash-toml"
elif [ -d "$REPLY/lib" ]; then
	BASH_TOML_LIB_DIR="$REPLY/lib"
else
	echo "Error: Could not determine \$BASH_TOML_LIB_DIR"
	exit 1
fi

# shellcheck disable=SC1091
source "$BASH_TOML_LIB_DIR/cmd/bash-toml.sh"
